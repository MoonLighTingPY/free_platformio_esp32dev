...
void loop() {
    ...
    if (myServo.attached()) {
        ...
        if (tag_mode == 0) {
            ...
            // Map sp1 (assumed range 0-100 or similar) to servo update delay
            // Higher sp1 = faster servo movement (lower delay)
            
            // Assuming sp1 is in range 0-100
            float sp1_clamped = tag_sp1;
            if (sp1_clamped < 0) sp1_clamped = 0;
            if (sp1_clamped > 100) sp1_clamped = 100;
            
            // Map sp1 to delay: sp1=0 -> slowest (50ms), sp1=100 -> fastest (1ms)
            int stepDelay = map((int)sp1_clamped, 0, 100, 50, 1);
